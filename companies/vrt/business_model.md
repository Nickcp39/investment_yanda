...
- **Why VRT remains well-placed?** Owns the integration/reference-design layer + breadth + speed; extending to 800V DC; #1 (tied) liquid-cooling share (~11.3%).
...
| Reinvest at attractive returns? | Yes — positive ROIC today; net cash funds bolt-on thermal M&A + buybacks | C028 | 4 |
| Inside circle of competence? | Requires underwriting the hyperscaler capex cycle (highest-beta name) | step0_plan | 3 |

**Net:** a genuinely **high-quality** business — real pricing power, expanding margins, strong cash conversion, a defensible integration moat. It is *not* a low-quality cyclical. The two things that keep it from a clean CORE buy are **price** (~48x forward, `valuation.md`) and **cycle exposure with no floor** (~75% downstream of four hyperscalers' discretionary capex, `bottleneck_map.md`) — plus the one disclosure blemish (withheld orders/B2B). Quality is high; the gate is valuation + beta + transparency, not the business.
