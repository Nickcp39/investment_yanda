...
**状态**: ⏸ PAUSED(batch-1 完成 5/26;batch-2 半成品 5;batch 3-5 未开)
...
| 2 | EW | Edwards 爱德华 | 器械(结构性心脏) | PARTIAL | 94.37 | 有卡·缺freshness |
| 2 | RMD | ResMed 瑞思迈 | 器械(呼吸/睡眠) | PARTIAL | 209.63 | 有卡·缺freshness |
| 2 | IDXX | Idexx | 宠物诊断 | PARTIAL | 557.80 | 有卡·缺freshness |
| 2 | DXCM | Dexcom | 器械(CGM 血糖) | PARTIAL | 71.25 | 有valuation·缺卡 |
| 2 | ALGN | Align 隐适美 | 器械(齿科) | PARTIAL | 184.52 | 缺valuation+卡 |
...
- 2026-07-05 **batch-2 API 断流中断 → PAUSED**。EW/RMD/IDXX 有 decision_card 但**缺 freshness.json**(续跑=补 manifest + 过门);DXCM 有 valuation 缺卡;ALGN 缺 valuation+卡。**batch 3-5 未启动。以后再做:读本 PLAN,先收尾 batch-2 五个 PARTIAL(或删半成品重跑、换新 as_of 重核价),再按队列推进。半成品未过 freshness 门,勿当结论用。**
