...
旧的 `companies/_template.md` 仍保留用作单文件式简报的模板。新研究<em>不</em>
默认走 souls pipeline — 见下方触发规则。

---

## 何时跑这个 pipeline (显式触发制)

**默认不跑。** 提到一家新公司、贴一段新闻、问"怎么看 X"——这些都<em>不</em>
触发 pipeline。默认是普通对话或单灵魂视角。

只有用户<em>明确要求</em>才跑全流程, 触发词例如:

- "请灵魂 / souls 研究 X"
- "跑 souls pipeline"
- "出 X 的简报 / brief / memo"
- "完整 panel / 全 panel 看 X"
- "走一遍 6 步流程"

模糊时<em>问一句</em>: "要 1-2 个灵魂的轻量看法, 还是跑完整 pipeline 出简报?"
不要自作主张直接进 Step 0。

### 不用 pipeline 的常见情况

- **快速看一眼**: 直接问灵魂中的某 1-2 位即可。例如:
"用 Marks 视角看一下 BTC 当前周期位置" — 不开 panel。
- **明确不在能力圈**: 段永平直接说"不为", 不需要 5 灵魂背书。
- **板块 / 主题分析**: 这套是为<em>单标的</em>设计的。板块用另一套
(以后写)。
- **闲聊 / 资料整理 / 信息检索**: 跟 pipeline 无关, 别强行启动。
